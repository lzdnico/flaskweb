...
def index():
    ...
    if request.method == "POST":
        ...
        if custom == '':
            ...
            CustomGroupClash = 'http://{ip}/clashr/test?sublink={sub}&custom={custom}&selectfirst=no'.format(ip=api.aff.apiip,sub=str(sub),custom=str(custom))
...
@app.route('/clashr/testload', methods=['GET', 'POST'])
def clashapitestload():
    try:
        ip = request.remote_addr
        sub = request.args.get('sublink')
        #print(sub)
